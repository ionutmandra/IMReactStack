...
export function leave_header(ref, callback, transition) {
    let container = dom.findDOMNode(ref), $container = $(container).addClass('detach-header'), height = $window.height();
    console.warn('extractDOMElements', ref, container);
    let slide = '.slide-' + Math.round(height * 4 / $window.scrollTop());
    let elements = {
        container: container,
...
export function enter_header(ref, callback, transition) {
    ...
    //Animation
    let tl1 = new TimelineLite({ onComplete: () => { callback(); $('body').css('overflow', 'visible'); } })
        .add(_.filter([
            TweenMax.to(line, 1, {
